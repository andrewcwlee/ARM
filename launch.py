...
def run_seed(cfg: DictConfig, env, cams, train_device, env_device, seed) -> None:
    ...
    if cfg.method.name == 'OTA':
        ...
        # Affordance-based Dual-arm Active Visual Manipulation
    ...
    elif cfg.method.name == 'BCA':
        if train_envs > 0:
            logging.warning('Training envs set to 0 for BC.')
            train_envs = 0
        replay_ratio = None  # No need for replay ratio for BC.
        explore_replay = bc_active.launch_utils.create_replay(
            cfg.replay.batch_size, cfg.replay.timesteps,
            cfg.replay.prioritisation,
            replay_path if cfg.replay.use_disk else None, env)
        replays = [explore_replay]
        bc_active.launch_utils.fill_replay(
            explore_replay, cfg.rlbench.task, env, cfg.rlbench.demos,
            cfg.method.demo_augmentation, 
            cfg.method.demo_augmentation_every_n,
            cfg.method.floating_viewpoint_env_bounds)
        agent = bc_active.launch_utils.create_agent(
        ...
        rg = BCFRolloutGenerator(scene_bounds=cfg.rlbench.scene_bounds,
                                   viewpoint_env_bounds=cfg.method.floating_viewpoint_env_bounds,
                                   device=train_device)
